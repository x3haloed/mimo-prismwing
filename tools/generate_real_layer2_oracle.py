#!/usr/bin/env python3
"""Generate the independent PW-0061 complete layer-2 trace."""
...
def load_input(manifest_path: Path) -> tuple[torch.Tensor, str]:
    ...
    record = manifest["captures"]["layer_01_final"]
    ...
    if record.get("shape") != [27, 4096] or record.get("dtype") != "BF16_widened_F32" or sha256(path) != record.get("sha256"):
        raise ValueError("PW-0060 layer-1 capture mismatch")
...
def generate(checkpoint_root: Path, verification: Path, source_manifest: Path,
    ...
    hidden, source_hash = load_input(source_manifest)
    ...
    prefix = "model.layers.2"; attention_prefix = f"{prefix}.self_attn"
    ...
    for expert in sorted(schedule):
        ...
        schedule_manifest.append({"expert": expert, "positions": positions}); safety.check(f"layer_2_expert_{expert}_complete")
    ...
    for name, value in (("expert_gate", torch.cat(gates)), ("expert_up", torch.cat(ups)),
        ...
        captures[name] = write_capture(output, name, value, safety)
    manifest = {"schema_version": 1, "semantic": "mimo_real_layer2_complete_oracle",
...
def main() -> int:
    ...
    generate(a.checkpoint, a.verification, a.source_manifest, a.output); return 0

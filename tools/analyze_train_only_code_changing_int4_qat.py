#!/usr/bin/env python3
"""Validate PW-0144's immutable code-changing INT4 QAT result."""
...
try:
    ...
    from run_train_only_code_changing_int4_qat import (
        LEARNING_RATE,
        REGULARIZATION,
        SAFETY_INTERVAL,
        SAMPLES,
...
SOURCE_SHA256 = "8828db18f3d9471aa9abd2110b78994b86803ff393e4ec0d9fe81e10cef5d00c"


def analyze(source_path: Path) -> dict:
    if sha256_file(source_path) != SOURCE_SHA256:
        raise ValueError("PW-0144 source report hash mismatch")
    source = json.loads(source_path.read_text())
    expected_steps = [0, *range(SAFETY_INTERVAL, STEPS + 1, SAFETY_INTERVAL)]
    if (
        source.get("schema_version") != 1
        or source.get("evidence_class") != "pw0144_train_only_code_changing_int4_qat"
        ...
        or source.get("decision") != "reject_fixed_grid_code_changing_qat"
    ):
        raise ValueError("PW-0144 source authority mismatch")
    ...
    if (
        ...
        or contract.get("bias_correction") is not True
        or contract.get("full_batch") is not True
        or contract.get("straight_through") is not True
        or contract.get("fixed_f16_grid_metadata") is not True
    ):
        raise ValueError("PW-0144 training contract mismatch")
    reports = source.get("reports", [])
    if [(row.get("layer"), row.get("expert")) for row in reports] != [
        (layer, expert) for layer, expert, _, _ in SAMPLES
    ]:
        raise ValueError("PW-0144 expert identity mismatch")
    for report in reports:
        history = report.get("training", {}).get("loss_history", [])
        if [row.get("step") for row in history] != expected_steps:
            raise ValueError("PW-0144 loss-history schedule mismatch")
        if report.get("initial_validation_metrics") != report.get("pw0139_validation_metrics"):
            raise ValueError("PW-0144 does not preserve PW-0139 validation authority")
        if not report.get("code_domain_valid") or not report.get("grid_metadata_unchanged"):
            raise ValueError("PW-0144 final artifact authority mismatch")
        if any(
            not isinstance(value, int) or value < 0
            for value in report.get("changed_codes", {}).values()
        ):
            raise ValueError("PW-0144 changed-code accounting mismatch")
        if len(report.get("initial_grid_digest", "")) != 64 or len(report.get("final_grid_digest", "")) != 64:
            raise ValueError("PW-0144 grid digest mismatch")
    recomputed = _gate(reports)
    if source.get("continuation_gate") != recomputed:
        raise ValueError("PW-0144 continuation gate mismatch")
    if recomputed["passes"] or any(row["passes"] for row in recomputed["experts"]):
        raise ValueError("PW-0144 rejection conflicts with gate")
    return {
        "schema_version": 1,
        "evidence_class": "pw0144_validated_code_changing_qat_rejection",
        ...
        "next_branch": "grid_changing_qat_or_different_executable_representation",

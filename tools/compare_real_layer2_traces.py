...
def compare(oracle_path: Path, rust_path: Path, target_layer: int = 2) -> dict:
    if target_layer not in (2, 4, 7, 11, 13, 14, 19, 29, 34): raise ValueError(f"unsupported routed trace layer {target_layer}")
...
def main() -> int:
    ...
    p.add_argument("--layer", choices=(2, 4, 7, 11, 13, 14, 19, 29, 34), default=2, type=int); a = p.parse_args()
